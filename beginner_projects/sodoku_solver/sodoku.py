def find_next_empty(puzzle):
    ...
    # keep in mind that we are using 0-8 our indices
    for r in range(9):
...
def solve_sudoku(puzzle):
    ...
    # step 1.1: if theres nowhere left, then were done because we only allowed valid inputs
    if row is None:
        return True

    # step 2: if theres a place to put a number then make a guess between 1 and 9
    for guess in range(1,10): #1-9
        ...
        if is_valid(puzzle, guess, row, col):
            ...
            # now recurse using the this puzzle
            # step 4 recursively call our function
            if solve_sudoku(puzzle):
                return True

        # step 5L if not valid OR if our guess does not solve the puzzle, then we need to backtrack
