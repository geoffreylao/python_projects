...
class GeniusComputerPlayer(Player):
    ...
    def get_move(self,game):
        if len(game.available_moves()) == 9:
            ...
            # get the square based off the mixmax algorithm
    ...
    def minimax(self, state, player):
        ...
        if player == max_player:
            best = {
                'position': None, 'score': -math.inf # each score should maximize (be larger)
            }
        else: 
            best = {
                'position': None, 'score': math.inf # each score should minimize
            }
